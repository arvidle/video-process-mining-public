...
class TripletAdaptiveMarginLoss(_Loss):
    ...
    Args:
        ...
        reduction (string, optional): Specifies the reduction to apply to the visualization:
            ``'none'`` | ``'mean'`` | ``'sum'``. ``'none'``: no reduction will be applied,
            ``'mean'``: the sum of the visualization will be divided by the number of
            elements in the visualization, ``'sum'``: the visualization will be summed. Note: :attr:`size_average`
    ...
    >>> visualization = triplet_loss(anchor, positive, negative)
    >>> visualization.backward()
